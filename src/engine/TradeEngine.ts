...
export default class TradeEngine {
  ...
  private currency(from?: Devise, to?: Devise): CurrencyLimit | null {
    ...
    return (
      this._currency_limits.find(cl => cl.from == from && cl.to == to) || null
    );
  }
  ...
  private _fullfillOrder = async (config: TradeConfig) => {
    try {
      ...
      const price_to_sell_current_tick =
        price?.multipliedBy(config.sell_coef) || new BigNumber(0);
      const last_buy_complete = this.last(orders, 'buy');

      if (to_balance.isGreaterThan(configuration.minimumSizeTo)) {
        //in from
        console.log(
        ...
        if (
          ...
          console.log(
            ...
            )} total_balance:${total_balance}(${from_balance})`,
          );
        }

        const is_bigger_than_minimum =
          amount.comparedTo(configuration.minimumSizeTo) >= 0;
        ...
        while (number_decimals_price >= 0) {
          try {
            ...
            const final_price_to_buy = priceToBuy
              .decimalPlaces(number_decimals_price)
              .toNumber();
            console.log(
              `final_amount amount:${final_amount} final_price_to_buy ${final_price_to_buy}`,
            );

            await Cex.instance.place_order(
              config.to,
              config.from,
              'buy',
              final_amount,
            ...
            const new_orders = await this.ordersHolders.list(
              config.from,
              config.to,
            );
            console.log(
              'new orders := ',
              new_orders.map(o => o.str()),
            );
            return true;
          } catch (e) {
            if (
              `${e}`.indexOf('Invalid price') < 0 ||
              number_decimals_price == 0
            )
              throw e;
  ...
  private async manageSellingOrder(
    ...
    if (original_price.isNaN())
      throw 'ERROR with nan result while trying to sell';
    // we recompute the original price to get the actual expected sell_coef
    const expected_price_to_sell = original_price.multipliedBy(
      config.sell_coef,
    );
    // and now we set the greater price - in case price dropped unexpectendly
    const sell_price = price_to_sell_current_tick.isGreaterThan(
      expected_price_to_sell,
    )
      ? price_to_sell_current_tick
      : expected_price_to_sell;
    console.log(
      'total expected devise  :=',
      to_balance.multipliedBy(sell_price).toNumber(),
    );
    console.log(
      'current  selling price :=',
      price_to_sell_current_tick.toNumber(),
    );
    ...
    while (number_decimals_price >= 0) {
      try {
        ...
        if (`${e}`.indexOf('Invalid price') < 0 || number_decimals_price == 0)
          throw e;
  ...
  private _afterTickStarted = async () => {
    ...
    while (i < this.configs.length) {
      ...
      try {
        const [expected_value, current_value] = await this.expectedValue(
          config,
        );
        if (!array[from])
          array[from] = {
            expected_value: new BigNumber(0),
            current_value: new BigNumber(0),
          };
        array[from].expected_value =
          array[from].expected_value.plus(expected_value);
        array[from].current_value =
          array[from].current_value.plus(current_value);
      } catch (e) {}
      i++;
    }
    ...
    Object.keys(array).forEach(k => {
      ...
      console.log(
        `managing for ${k} ; expected := ${expected_value.toNumber()} ; current := ${current_value}`,
      );
    });
